...
class Relay(CoordinatorEntity, SwitchEntity):
    def __init__(
        self, coordinator: DataUpdateCoordinator, pin: int, device: DeviceInfo, api: Api
    ):
        ...
        self._attr_unique_id = (
            format_mac(device.get("serial_number")) + "-" + str(self._pin) + "-switch"
        )
    ...
    async def async_turn_on(self, **kwargs: Any):
        await self._api.call_api("preset.htm?set" + str(self._pin) + "=1")
        await self.coordinator.async_refresh()

    async def async_turn_off(self, **kwargs: Any):
        await self._api.call_api("preset.htm?set" + str(self._pin) + "=0")
        await self.coordinator.async_refresh()
